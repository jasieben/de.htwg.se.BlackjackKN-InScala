...
import de.htwg.se.blackjackKN.model.cardsComponent.{FaceCard, NumberCard, cardsBaseImpl}
import de.htwg.se.blackjackKN.model.{Ranks, Suits, cardsComponent}
  ...
  "A Controller" when {
    "observed by an Observer" should {
      ...
      "notify its Observer after standing" in {
        ...
        controller.player.addCardToHand(cardsBaseImpl.NumberCard(Suits.Clubs))
        controller.stand()
      ...
      "notify its Observer after hitting" in {
        ...
        controller.player.clearHand()
        controller.dealer.addCardToHand(NumberCard(Suits.Hearts, 7))
        controller.dealer.addCardToHand(cardsBaseImpl.NumberCard(Suits.Clubs))
        controller.player.addCardToHand(NumberCard(Suits.Hearts, 9))
        controller.player.addCardToHand(cardsBaseImpl.NumberCard(Suits.Clubs))
    ...
    "revealing cards" should {
      ...
      "make correct decision when dealer and player have blackjack" in {
        ...
        controller.dealer.clearHand()
        controller.player.clearHand()
        controller.dealer.addCardToHand(cardsBaseImpl.NumberCard(Suits.Hearts))
        controller.dealer.addCardToHand(FaceCard(Suits.Clubs))
        controller.player.addCardToHand(cardsBaseImpl.NumberCard(Suits.Hearts))
        controller.player.addCardToHand(cardsBaseImpl.FaceCard(Suits.Clubs))
        controller.evaluate()
      ...
      "make correct decision when dealer has blackjack and player not" in {
        ...
        controller.player.clearHand()
        controller.dealer.addCardToHand(cardsBaseImpl.NumberCard(Suits.Hearts))
        controller.dealer.addCardToHand(cardsBaseImpl.FaceCard(Suits.Clubs))
